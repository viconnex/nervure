...
@Path("tips")
public class TipPointsResource {

    private static final Logger logger = LoggerFactory.getLogger(TipPointsResource.class);
    ...
    @Inject
    public TipPointsResource(GraphHopper graphHopper, EncodingManager encodingManager, DelaunayTriangulationIsolineBuilder delaunayTriangulationIsolineBuilder) {
    ...
    public Response doGet(
        ...
        if ("polygon".equalsIgnoreCase(resultStr)) {
            List<Coordinate> tipsCoordinates = isochrone.searchGPSTips(qr.getClosestNode(), nBuckets);

            return Response.ok(tipsCoordinates).header("X-GH-Took", "" + sw.getSeconds() * 1000).
                    build();
