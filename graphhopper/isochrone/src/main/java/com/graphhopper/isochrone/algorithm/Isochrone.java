...
//        this.finishLimit = this.limit + Math.max(this.limit * 0.14, 200_000);
    ...
    public List<NodeEdgesCoordinates> searchGPS2(int from, final int bucketCount) {
        searchInternal(from);

        final List<NodeEdgesCoordinates> nodeEdgesCoordinates = new ArrayList<>();
        ...
        fromMap.forEach(new IntObjectProcedure<IsoLabel>() {
            ...
            public void apply(int nodeId, IsoLabel label) {
                if (label.parent != null) {
                    nodeEdgesCoordinates.add(
                            new NodeEdgesCoordinates(
                                    new Coordinate(na.getLatitude(label.parent.adjNode),na.getLongitude(label.parent.adjNode)),
                                    new Coordinate(na.getLatitude(label.adjNode),na.getLongitude(label.adjNode))
                            )
                    );
                }
            }
    ...
    public List<Coordinate> searchGPSTips(int from, final int bucketCount) {
        searchInternal(from);

        final List<Coordinate> tipCoordinates = new ArrayList<>();

        final NodeAccess na = graph.getNodeAccess();

        for (IsoLabel tip: fromHeap){
            tipCoordinates.add(new Coordinate(na.getLatitude(tip.adjNode), na.getLongitude(tip.adjNode)));
        }

        return tipCoordinates;
    }

    private void incrementNodeWeightRecursively(SPTEntry node){
        if (fromMapWeight.get(node.adjNode) != null){
            fromMapWeight.put(node.adjNode, fromMapWeight.get(node.adjNode) + 1);
        } else {
            fromMapWeight.put(node.adjNode, 1);
        }

        if (node.parent != null){
            incrementNodeWeightRecursively(node.parent);
        }
    }

    public List<NodeEdgesCoordinatesWithWeight> searchGPSNodeWeight(int from, final int bucketCount) {
        searchInternal(from);

        final List<Coordinate> tipCoordinates = new ArrayList<>();


        for (SPTEntry tip: fromHeap){
            incrementNodeWeightRecursively(tip);
        }

        final List<NodeEdgesCoordinatesWithWeight> nodeEdgesCoordinates = new ArrayList<>();
        final NodeAccess na = graph.getNodeAccess();

        fromMap.forEach(new IntObjectProcedure<IsoLabel>() {
            @Override
            public void apply(int nodeId, IsoLabel label) {
                if (label.parent == null || fromMapWeight.get(nodeId) == null){
                    return;
                }

                nodeEdgesCoordinates.add(
                        new NodeEdgesCoordinatesWithWeight(
                                fromMapWeight.get(nodeId),
                                new Coordinate(na.getLatitude(label.parent.adjNode),na.getLongitude(label.parent.adjNode)),
                                new Coordinate(na.getLatitude(label.adjNode),na.getLongitude(label.adjNode))
                        )
                );
